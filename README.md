...
Any single witness-chain RPC attempt under a deadline is additionally capped at the tightest of: half the witness stage budget, the global per-attempt timeout, and half of what the call still has as the attempt starts (recomputed after any permit wait) — so a stalled endpoint (or a saturated concurrency permit — waits are deadline-bounded too) can never consume the stage and leave nothing for another rotation, even when an old-block clamp, a slow R2 pre-try, or a long permit queue already shrank what is left.
