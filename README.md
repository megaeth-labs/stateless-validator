...
The pipeline cross-checks the handed number against the fetched block's own header, so a drifted (number, hash) pair fails typed as soon as the block lands — the doomed witness fetch is cancelled rather than retried to its deadline — instead of serving a witness routed by the wrong height.
