...
Frontier probes usually miss — the uploader typically lags the generator — and cost one fast 404; the frontier band is a small near-tip window (32 blocks of uploader-lag grace, far narrower than the 4096-block routing window), hits there are labeled `witness_r2_frontier` (vs `witness_r2` past the band) so their hit rate stays separable, and only an in-band `missing` is excluded from `debug_trace_r2_witness_errors_total{kind="missing"}` — past the band the object must exist, so the bucket-integrity alarm keeps covering recent-but-below-tip holes.
...
Any single witness-chain RPC attempt under a deadline is additionally capped at the tightest of: half the witness stage budget, the global per-attempt timeout, and half of what the stage has left when the chain starts — so a stalled endpoint (or a saturated concurrency permit — waits are deadline-bounded too) can never consume the stage and leave nothing for another rotation, even when an old-block clamp or a slow R2 pre-try already shrank the stage.
