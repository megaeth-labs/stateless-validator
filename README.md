...
The handed number is cross-checked against the served block's own header on every tier — cache hits included — so a drifted (number, hash) pair fails typed on warm and cold paths alike instead of serving a witness routed by the wrong height; on a fresh fetch the check lands as soon as the block arrives and the doomed witness fetch is cancelled rather than retried to its deadline.
