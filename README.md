...
`--r2-connections` (default 1) raises that ceiling by holding several clients and handing them out round-robin per attempt — which also means a retry leaves the connection that just failed, and a dropped connection no longer fails every GET riding it at once.
