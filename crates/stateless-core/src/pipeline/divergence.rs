...
pub trait DivergenceLookups {
    ...
    /// fatal corruption, so a store that also keeps non-contiguous older rows (e.g. the
    /// trace server's below-floor islands) must report its contiguity floor here, not its
    /// oldest row.
