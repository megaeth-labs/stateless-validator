...
On the request path a frontier block (above the local DB tip) gives the generator a bounded exclusive grace before rotating, since the fallback endpoints are fed by the same generation pipeline and cannot be ahead of it — a rotation on a frontier miss only buys a public-gateway round trip to learn what the generator already said; outcome observable via `debug_trace_frontier_grace_total{outcome}`.
Unlike chain sync's twin the request-path grace carries one meaning only: its freshness signal is the local DB tip, read fresh from redb per fetch, so there is no head-observation anchor that could go stale and silently disable the routing.
A frontier witness that never arrives surfaces as `-32002 witness ... is not generated yet; retry shortly` (`DataProviderError::WitnessNotReady`, reason label `witness_not_ready`) rather than the generic `-32001` deadline, so a client can tell a retryable chain-lag apart from a nonexistent block and from an overload signal.
