...
HTTP responses negotiate gzip/zstd compression per request via `Accept-Encoding` (kill switch: `--response-compression-disabled`); bodies ≤ 4 KiB stay identity, the stack order lives in `http_middleware()` (compression outermost keeps `x-response-size` at the uncompressed size and `x-execution-time-ns` free of compression CPU), and response-cache hits currently re-compress per hit (compressed-variant caching is the known follow-up).
