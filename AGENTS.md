...
The count is published as `debug_trace_r2_connections` / `r2_connections`, is rejected by name at zero and on the S3 target (HTTP/1.1 already opens a socket per in-flight GET there), and on the validator the shared semaphore is `--witness-max-concurrent-requests`, which sizes the RPC gateway too, so the two consumers trade off against each other.
