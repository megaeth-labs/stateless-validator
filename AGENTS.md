...
`--historical-readahead <N>` prefetches the next N blocks after each historical by-number request through the same pipeline into the block-data cache (bounded, best-effort, dedup'd), so in-order backfill crawls mostly hit warm data.
