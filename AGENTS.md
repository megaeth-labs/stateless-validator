...
Requests the framework answers before any handler runs (unknown method, malformed top-level params, unparseable batch entries) are folded in by the same middleware as the balanced pair `shape="rejected"` / `reason="rejected"`, and the opts-less `trace_*` methods record their `default` arrival at handler entry; the only deliberate approximation is a batch the server itself aborts over the response-size cap (its killed entries record no outcome rather than masquerade as client hangups) and batch entries that never started when the connection died (on neither side, so `cancelled` undercounts torn-down oversized batches).
