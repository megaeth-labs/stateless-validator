...
Both binaries serve witnesses out of the same bucket, filled by the same uploader, so "is this absent object expected or a hole?" has one answer and one rule: `r2_band` / `R2Band` in `stateless-common`, two bands, anchored on each reader's best estimate of the **chain head**.
...
So the validator anchors on its last polled remote head and the trace server on `DataProvider::tip_hint` (the monotonic maximum on-chain height it has observed), rather than on its local DB tip as it once did.
