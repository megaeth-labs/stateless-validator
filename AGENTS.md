...
HTTP responses negotiate gzip/zstd compression per request via `Accept-Encoding` (kill switch: `--response-compression-disabled`); the compression layer sits outside the size/timing middleware, so `x-response-size` stays the uncompressed size and `x-execution-time-ns` excludes compression CPU.
