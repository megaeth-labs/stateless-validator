...
It only stores idempotent request shapes: the six supported built-in tracers (keyed by their parsed typed `tracerConfig`) and the bare default struct-logger request; JS tracers, `muxTracer`, and struct-logger requests with non-default flags bypass it entirely; a type-malformed `tracerConfig` on a config-reading builtin (call/prestate/flatCall/erc7562) is rejected with `-32602` before any block data is fetched.
