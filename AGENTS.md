...
`--r2-max-concurrent-requests` should stay at or below the edge's per-connection stream limit (Cloudflare's is 100), since GETs above it queue inside the HTTP/2 connection where the wait escapes `debug_trace_r2_witness_queue_wait_seconds` and counts against the per-attempt timeout.
