...
`--max-connections` (default 100, jsonrpsee's own) caps simultaneous inbound connections and answers the rest `429` before any handler runs; it is the server's only admission control, and what it actually bounds depends on the transport — HTTP/1.1 carries one request per connection at a time, so it currently acts as a concurrency limit, whereas one HTTP/2 connection would pass it carrying many concurrent requests.
